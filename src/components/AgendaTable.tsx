  ...
  return (
    ...
    <div className="overflow-auto rounded-lg border border-border bg-card shadow-sm">
      <Table>
        ...
        <TableBody>
          {items.map((item, idx) => (
            <TableRow key={item.id} className={`transition-colors hover:bg-primary/10 ${!item.motorista ? 'bg-red-200 dark:bg-red-900/40' : idx % 2 === 1 ? 'bg-yellow-50/60 dark:bg-yellow-900/10' : tipoRowColor(item.tipo)}`}>
