        ...
        <TableBody>
          {items.map((item, idx) => (
            <TableRow key={item.id} className={`transition-colors hover:bg-primary/10 ${!item.corManual ? (idx % 2 === 1 ? 'bg-yellow-50/60 dark:bg-yellow-900/10' : tipoRowColor(item.tipo)) : ''}`} style={item.corManual ? { backgroundColor: item.corManual } : undefined}>
              ...
              {canViewFinancials && (
                <TableCell className={`px-0.5 py-0 text-right font-mono text-[9px] text-muted-foreground truncate ${!item.custo && !item.fornecedor.toLowerCase().includes("executive") && !(item.motorista && items.some(other => other.id !== item.id && other.motorista === item.motorista && other.data === item.data && (other as any).formaContratacao === 'diaria' && other.custo > 0)) ? 'bg-orange-200 dark:bg-orange-900/40' : ''}`}>
