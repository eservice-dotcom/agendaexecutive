...
const CadastroCentrosCusto = () => {
  ...
  const handleAdd = async () => {
    ...
    if (!nome) { toast.error("Digite o nome do centro de custo."); return; }
    if (items.some((i) => i.nome.toLowerCase() === nome.toLowerCase())) {
      toast.error("Esse centro de custo já existe."); return;
    }
  ...
  const handleAddSub = async (centroId: string) => {
    ...
    const existing = subgrupos.filter((s) => s.centro_custo_id === centroId);
    if (existing.some((s) => s.nome.toLowerCase() === nome.toLowerCase())) {
      toast.error("Subgrupo já existe neste centro."); return;
    }
    const session = await getSession();
    if (!session) return;
    const { error } = await supabase.from("subgrupos_custo").insert({ nome, centro_custo_id: centroId, user_id: session.user.id });
  ...
  const handleDeleteSub = async (id: string) => {
    const { error } = await supabase.from("subgrupos_custo").delete().eq("id", id);
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Centros de Custo</h3>
      ...
      <div className="rounded-md border border-border">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>Centro de Custo / Subgrupos</TableHead>
...
export default CadastroCentrosCusto;
